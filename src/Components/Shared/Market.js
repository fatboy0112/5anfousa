...
const Market = React.memo((props) => {
    ...
    Bets = filter(Bets, (bet) => bet.Status === 1);
    let slicedBets = null;
    ...
    let count = 0;
    let classForHandicap = Market.Id === 13 ? 'handicap' : '';
        ...
        ? slicedBets.map((flex) => {
              ...
              return (
                  <Grid container className={`mx-0 ${classForHandicap}`} key={count}>
                     {
                         Market.Id === 13 && <Grid item xs className={`ripple-bet anim handicap small-width`} >
                         {`(${flex[0].BaseLine})`}
                     </Grid> 
                     } 
                      {flex.map((item) => {
        ...
        slicedBets.length > 0 && (
            <div className="mx-auto match__bets market__odds pt-2">
                <span className="odd__title">
                    { Market.Id ===13 ? `Handicap`: (Translate.markets[Market.Id] )} {line}
                </span>
                {drawBets}
            </div>
        )
